...
type_declaration:               type_declaration SEMICOLON ID EQUSIGN  type
								{
									...
									type_declarations+="typedef ";
                                    type_declarations+=$5->kind ;
                                    type_declarations+=$3;
                                    type_declarations+=";";
                                    type_declarations+="\n";
...
type:                           standard_type
                                ...
                                | RECORD record_body END
								{
									...
									/*-----以上为语义分析部分，已完成--------*/
									out << "struct{\n";
                                    for(std::map<std::string, typeAttrip>::iterator iter=$2->begin();iter!=$2->end();iter++)
                                    {
                                        out << iter->second.kind;
                                        out << iter->first;
                                    }
                                    /*-------code generation-------*/
								}
